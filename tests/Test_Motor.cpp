...
const int MAXDEGREEPOLY=112; //57 for coeffs1,  112 for coeffs2 or 3
...
void polyEval(double coeffs[], double *x, double *output){
	...
	for(i=0;i<MAXDEGREEPOLY-1;i++){
		result+=coeffs[i]*pow(*x,MAXDEGREEPOLY-1-i);
	}
...
void *testThread1(void *ptr) {
  ...
  while(ticks_t1<TIME_MAX+1) {
  	...
  	/* do the stuff */
  	if(ticks_t1%100==0){
  		polyEval(coeffs3, &timePolyEval, &angleTest);
